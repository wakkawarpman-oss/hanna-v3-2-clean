...
def _export_result_artifacts(
    ...
    if not export_formats:
        return {}
    # Validate adapter outcomes against the canonical AdapterResult schema before
    # writing any artefacts.  Malformed outcomes raise ValueError with a
    # machine-readable JSON payload so callers can surface a structured error.
    raw_outcomes = [o.to_dict() for o in getattr(result, "outcomes", [])]
    if raw_outcomes:
